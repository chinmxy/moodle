...
<body>
  ...
  <div class="container-fluid p-3 mb-2 bg-white text-dark">
    ...
    <div class="container-fluid p-3 mb-2 bg-light text-dark">
      ...
      $s = "select * from teacher_status where teacher_id = $currentid";
      $result = mysqli_query($conn, $s);
      if (mysqli_num_rows($result)) {
        while ($row = mysqli_fetch_assoc($result)) {
          echo "Your application for " . $row['course_name'] . " was " . $row['status'] . ". </br>";
        }
      } else {
        $s = "select * from pr_teacher where teacher_id = $currentid";
        $result = mysqli_query($conn, $s);
        if (mysqli_num_rows($result)) {
          echo "Please wait for few days. Your application is being processed.";
        } else {
          echo "Not applied for any courses";
        }
      }
      ?>
    </div>
