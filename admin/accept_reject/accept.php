...
<body>
  ...
  <div id='requests'>
    ...
    if (mysqli_num_rows($result) > 0) {
      ...
      echo "<table><tr><th>Roll No.</th><th>Course</th></tr>";


      while ($row = mysqli_fetch_assoc($result)) {
        $_SESSION['course'] = $row["course_name"];
        echo "<tr><td>" . $row["rollno"] . "</td><td>" . $row["course_name"] . "</td><td>" . "<button type='button' name='accept' value='Accept' class='btn btn-primary' id=$i style='padding: 5px 24px;' onclick='accept_student($i)'>ACCEPT</button>" . "   " .  "<button type='button' value='Reject' class='btn btn-danger' id = $j style='padding: 5px 24px;' onclick='reject_student($j)'>REJECT</button>" . "</td></tr>";
        $dict[$i] = $row['rollno'];
        $i += 1;
  ...
  <script language='javascript'>
    ...
    function accept_student(x, course) {
      var xhttp;
