...
$sql = "select rollno,course_name from pr_student";
...
if (mysqli_num_rows($result) > 0) {
  ...
  echo "<table><tr><th>Roll No.</th><th>Subject</th></tr>";


  while($row = mysqli_fetch_assoc($result)) {
    $subject=strval($row['course_name']);
  echo "<tr><td>" . $row["rollno"]. "</td><td id='sub$i'>". $row["course_name"]."</td><td>". "<button type='button' name='accept' value='Accept' class='btn btn-primary' id=$i style='padding: 5px 24px;' onclick='accept_student($i)'>ACCEPT</button>". "   ".  "<button type='button' value='Reject' class='btn btn-danger' id = $j style='padding: 5px 24px;' onclick='reject_student($j)'>REJECT</button>"."</td></tr>";
  $dict[$i] = $row['rollno'] ;
  $i += 1;
...
function accept_student(x) {
  var xhttp;
      ...
      console.log(sub);
			...
			xhttp.open("GET","accept_student.php?student_id=" +x+"subject"+sub , true);
			xhttp.send(null);
...
<!-- Optional JavaScript -->
    ...
    <script>
function accept_student1(x){
  console.log('sdknbda');
// $.ajax({
//   url: "accept_student.php?student_id="+x ,
